        ...
        children: [
          SizedBox(
            ...
            child: ListView(
              ...
              children: [
                for (final entry in const {'All': null, 'Pending': 'pending', 'Received': 'received', 'Invoiced': 'invoiced', 'Ready': 'ready_for_shipment', 'Processing': 'processing', 'Completed': 'completed', 'Cancelled': 'cancelled'}.entries)
                  Padding(
                    padding: const EdgeInsetsDirectional.only(end: 8),
                    child: ChoiceChip(label: Text(entry.key), selected: ref.watch(orderFilterProvider) == entry.value, onSelected: (_) => ref.read(orderFilterProvider.notifier).state = entry.value),
                  ),
              ],
            ),
          ),
          Expanded(
            child: orders.when(
              ...
              error: (error, _) => ErrorView(message: 'Unable to load Orders. Please try again.', retryLabel: 'Retry', onRetry: () => ref.invalidate(ordersProvider)),
              data: (page) => page.results.isEmpty
                  ...
                  : RefreshIndicator(
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) => OrderCard(order: page.results[index], onTap: () => context.goNamed(RouteNames.orderDetail, pathParameters: {'id': '${page.results[index].id}'})),
                      ),
                    ),
            ),
          ),
    ...
    return AppScaffold(
      ...
      child: order.when(
        ...
        data: (data) => ListView(
          padding: const EdgeInsets.all(16),
          children: [
            Card(child: ListTile(title: Directionality(textDirection: TextDirection.ltr, child: Text(data.orderNumber, style: const TextStyle(fontWeight: FontWeight.w900))), subtitle: Text(formatDate(data.createdAt)), trailing: StatusBadge(status: data.status))),
